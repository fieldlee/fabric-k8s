...
def configORGS(name, path): # name means if of org, path describe where is the namespace yaml to be created.
	...
	if path.find("peerOrganizations") != -1:
		dataPath = path.replace("transform/../crypto-config/peerOrganizations/", "/opt/nfs/fabric/peer/")
	elif path.find("ordererOrganizations") != -1:
		dataPath = path.replace("transform/../crypto-config/ordererOrganizations/", "/opt/nfs/fabric/orderer/")
